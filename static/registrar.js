...
async function fetchData() {
  try {
    const response = await fetch('/get_submitted_subjects');
...
async function approveAllSubjects(professorId) {
  ...
  try {
    const response = await fetch('/approve_all_subjects', {
      method: 'POST',
...
async function denySubject(item, professorId) {
  try {
    ...
    const res = await fetch("/deny_subject", {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function fetchDeniedSubjects() {
  try {
    const res = await fetch('/get_denied_subjects');
    if (!res.ok) throw new Error('Network');
